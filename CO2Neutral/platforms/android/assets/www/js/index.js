...
var app = {
	...
	var km_trayectoAutubus = document.getElementById('km_trayectoAutubus').value;
	var n_vecesAutubus = document.getElementById('n_vecesAutubus').value;
	...
	for (var i = 0; i < total_medios.length; i++) {		
		//alert(apiAccess.getPeriodicidadValor(selectedValueArray[i])).valor;
			if (total_medios[i].nombre == "Pie"){
				alert(selectedValueArray[i]);
				...
				document.getElementById('total_Automóvil').value= (km_trayectoAutomóviln*n_vecesAutomóvil*(total_medios[i].valor))/(selectedValueArray[i]);
			}
			else {					
